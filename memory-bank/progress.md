...
| | 第 8 步：历史记录流与筛选 | ✅ 已完成（列表+卡片+滑动+动画+到达时间+筛选+回到顶部） |
| 第五阶段：成就感与图库 | 第 9 步：车型照片映射系统 | 🔶 部分完成（目录结构+Image.asset+errorBuilder回退，零照片可运行） |
| | 第 10 步：成就统计页面 | 🔶 部分完成（2×2网格+总运转时长+领航者勋章已完成，收集者进度条+车型网格开发中） |
...
### 2026-04-13 (第十七次更新) — 席位过滤修复 + 收集者页面开发

**席位过滤 Bug 修复**
- `entry_page.dart`：`ValueKey` 从仅含车型类型改为含 `车型类型_席位类别_列表长度` 三元组
- Coach 模式下正确隐藏 EMU 专有席位，车型切换时 DropdownButtonFormField 强制重建

**收集者页面开发**
- `achievement_page.dart`：`_CollectorPage` 完全重写
- 同时监听 `tripListProvider` + `trainHierarchyProvider`，获取全部车型列表
- 进度卡片：`LinearProgressIndicator` 百分比进度条 + 已收集/总数统计
- 车型网格：Wrap 布局，已收集高亮（实心图标+primaryContainer），未收集灰色（空心图标+surfaceContainerLow）
- 车型计数：CR/CRH 按系列（L3）计数，Coach 按平台（L2）计数

**文档更新**
- `progress.md`：第 8 步标记 ✅ 完成，第 10 步修正为 🔶 部分完成
- `handover-notes.md`：移除已修复的 AnimatedSize 和席位过滤 Bug，更新下一步计划

### 2026-04-13 (第十六次更新) — 代码审查优化
