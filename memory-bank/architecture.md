...
│   ├── app/
│   │   └── build.gradle.kts    # Android SDK 版本配置 (targetSdk 34, compileSdk 34)
...
│   ├── models/                 # 数据模型层（Trip, TrainModel, Seat, Operator 等）
│   ├── providers/              # Riverpod 状态管理层（AsyncNotifierProvider）
...
| Android SDK | minSdk 21 / targetSdk 34 / compileSdk 34 | 覆盖 Android 5.0+，适配 Android 14 |
...
### 待引入（后续步骤需要时添加）
- `objectbox / objectbox_flutter_libs` — 第 5 步数据库集成时引入
